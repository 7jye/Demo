...
namespace Student.Domain
{
	...
	public partial interface IDemoDbContext
	{
		...
		T ExecuteScalar<T>(string sql, object parameter = null);

		Task<T> ExecuteScalarAsync<T>(string sql, object parameter = null);

		IEnumerable<T> Query<T>(string sql, object parameter = null);

		Task<IEnumerable<T>> QueryAsync<T>(string sql, object parameter = null);

		int Execute(string sql, object parameter = null);
		#endregion
	}
}
